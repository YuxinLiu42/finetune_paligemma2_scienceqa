...
|---|---|---|
| `model.base_learning_rate` | 1.33e-4 | yes (log-uniform 7e-5 to 2e-4) |
...
| sandy-sweep-7 | n/a | 0.714 | 1.33e-4 | 4 (16) | killed by billing (top val, no test) |
...
3. `GET /monitor/drift` compares the reference against that production table
   over the columns both share, returns Evidently's verdict together with the
